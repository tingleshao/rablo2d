...
class Field
  ...
  def render_interp_spokes(shiftx, shifty, color)
    ...
    @interpBegin.each_with_index do |p, i|
       @app.line(p[0]+shiftx, p[1]+shifty, interpEnd[i][0]+shiftx, interpEnd[i][0]+shifty)
    end
  end
	...
	button("Save") {
 	...
 	button("interpolate at index 1") {
	  ...
	  file = File.open('interpolated_logrkm1s_0', 'r')
          ...
          indices << xt.length-1
          ## ---  
          vt = [xt[indices[1]+1] - xt[indices[1]], yt[indices[1]+1] - yt[indices[1]]]
          size_vt = vt[0]**2 + vt[1]**2
          norm_vt = vt.collect{|v| v / size_vt} 
          
          kt = ( 1 + ( -1 * Math.exp(logrkm1[indices[1]]) ) ) / rt[indices[1]] 
          dt = 0.01
          u01 = interpolateSpokeAtPos1($u1, norm_vt, kt, dt)
     
          $interpBegin << [100+vt[0]*0.01,75+vt[1]*0.01]
          $interpEnd <<  [100+vt[0]*0.01+u01[0]*rt[indices[1]],75+vt[1]*0.01+u01[1]*rt[indices[1]]]
...
def initialConfig
  ...
  srep0.index = 0;
  $sreps = [srep0]
  $u1 = srep0.atoms[1].spoke_direction[0]
  @shifts = [100, 100, 100]
  $interpBegin = []
  $interpEnd = []
